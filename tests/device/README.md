...
| `sdget_demo.py` | Serial-only demo: finds the newest SD log (the open one, else the highest `LOGnnnn.CSV`), downloads it in CRC-checked `SDGET` chunks into a variable, splits it into sessions at each `# stimjimAWG log` header, and warns when trains ran during the transfer or the file grew past the snapshot. `--save <file|dir>` writes it out through `<file>.part` and an atomic rename (no `fsync`, so the Pi's other card users are never stalled; see `SafeWriter`), `--write end` once complete or `--write chunk` as each chunk passes its CRC, so a failed transfer keeps what arrived. `--set-clock` first sets the board's clock to the host's UTC time (`set_clock`, importable: half the fastest `CLK?` round trip is added to the value sent, then a read-back reports the offset), and warns on Linux when `timedatectl` says the host clock is not NTP-synchronized — a Pi without network runs on its last shutdown time. `python sdget_demo.py /dev/ttyACM0 --save logs/ --write chunk` |
