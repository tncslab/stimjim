...
`bench_arm.py` is what sizes `CAL STARTLAT`, so run it on a binary whose `IDN` line you have
recorded: `hot=RAM` and `hot=flash` builds give different arm costs, and comparing the two is how
